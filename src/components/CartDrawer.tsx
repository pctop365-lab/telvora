...
export default function CartDrawer() {
  ...
  const { items, count, subtotal, delivery, total, updateQuantity, removeFromCart } = useCart();
  ...
  return (
    <div className="fixed inset-0 z-[65]">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-md bg-graphite-800 border-l border-white/10 flex flex-col animate-slide-in-right">
        <div className="flex items-center justify-between p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-display font-bold text-lg text-white">Корзина</h3>
        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center px-6 text-center">
            <div className="w-20 h-20 rounded-3xl bg-white/5 flex items-center justify-center mb-4">
              <ShoppingBag className="w-10 h-10 text-graphite-600" />
            </div>
          ...
          <>
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="flex gap-4 p-3 bg-graphite-900 rounded-2xl border border-white/5"
                >
                  <div className="w-20 h-20 rounded-xl overflow-hidden bg-graphite-800 shrink-0">
                    <img
                      src={item.image}
                      alt={item.name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-2 bg-graphite-800 rounded-lg p-1">
                        <button
                          onClick={() => updateQuantity(item.id, -1)}
                          className="w-7 h-7 flex items-center justify-center text-graphite-300 hover:text-white hover:bg-white/10 rounded-md transition-colors"
                        >
            ...
            <div className="border-t border-white/10 p-6 space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-graphite-400">Сумма товаров</span>
              ...
              {subtotal > 0 && subtotal < siteContent.freeDeliveryThreshold && (
                <p className="text-xs text-accent-500">
                  Добавьте товаров ещё на {formatPrice(siteContent.freeDeliveryThreshold - subtotal)} для бесплатной доставки
                </p>
              )}
