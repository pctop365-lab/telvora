...
function spawnRun(args, env = {}) {
  const quote = value => `'${value.replaceAll("'", "'\\''")}'`;
  const mapped = [posix(script), ...args.map(posix)];
  return spawn(bash, ['-lc', mapped.map(quote).join(' ')], { cwd: root, encoding: 'utf8', env: { ...process.env, ...(process.platform === 'win32' ? { TELVORA_NO_REALPATH: '1' } : {}), ...env } });
}
...
const productionOverride = await fixture(); const productionBefore = await treeSnapshot(productionOverride.doc); result = run(await argsFor(productionOverride, ['--dry-run']), { TELVORA_SEO_LOCK_FILE: posix(join(productionOverride.base, 'alternate.lock')) }); assert.notEqual(result.status, 0, 'production lock override was accepted'); await assertTreeRestored(productionOverride.doc, productionBefore, 'production lock override mutated DocumentRoot'); result = run(await argsFor(productionOverride, ['--dry-run']), { TELVORA_DEPLOY_TEST_MODE: '1', TELVORA_SEO_LOCK_FILE: posix(join(productionOverride.base, 'test.lock')) }); assert.equal(result.status, 0, 'test-only lock override was rejected'); await rm(productionOverride.base, { recursive: true, force: true });
const productionHooks = await fixture(); const hooksBefore = await treeSnapshot(productionHooks.doc); result = run(await argsFor(productionHooks, ['--dry-run']), { TELVORA_DEPLOY_TEST_HOLD_AFTER_LOCK_SECS: '1', TELVORA_DEPLOY_TEST_READY_FILE: join(productionHooks.base, 'unexpected.ready') }); assert.notEqual(result.status, 0, 'production test hooks were accepted'); await assertTreeRestored(productionHooks.doc, hooksBefore, 'production test hooks mutated DocumentRoot'); await rm(productionHooks.base, { recursive: true, force: true });
...
if (process.platform !== 'win32') {
  ...
  const locked = await fixture(); const ready = join(locked.base, 'holder.ready'); const holderStaging = join(locked.base, 'holder-staging'); const holderBackups = join(locked.base, 'holder-backups'); const holder = spawnRun(await argsFor({ ...locked, staging: holderStaging, backups: holderBackups }, ['--dry-run']), { TELVORA_DEPLOY_TEST_MODE: '1', TELVORA_DEPLOY_DEBUG: '1', TELVORA_DEPLOY_TEST_HOLD_AFTER_LOCK_SECS: '4', TELVORA_DEPLOY_TEST_READY_FILE: ready });
  const beforeLocked = await treeSnapshot(locked.doc); for (let i = 0; i < 80; i += 1) { try { await stat(ready); break; } catch { await new Promise(resolve => setTimeout(resolve, 50)); } } await stat(ready);
  result = run(await argsFor(locked, ['--dry-run']), { TELVORA_DEPLOY_DEBUG: '1' }); assert.notEqual(result.status, 0, 'concurrent lock was not rejected'); await assertTreeRestored(locked.doc, beforeLocked, 'rejected concurrent run mutated DocumentRoot'); result = run(['--rollback', join(locked.backups, 'not-created'), '--document-root', locked.doc, '--staging-root', locked.staging, '--backup-root', locked.backups], { TELVORA_DEPLOY_DEBUG: '1' }); assert.notEqual(result.status, 0, 'manual rollback was not rejected while deployment lock was held');
