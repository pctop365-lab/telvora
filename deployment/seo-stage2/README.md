...
The production lock is always `dirname(canonical DocumentRoot)/.telvora-seo-deploy.lock`; staging roots and archive names cannot change its identity. `TELVORA_SEO_LOCK_FILE` is accepted only in explicit test mode and is rejected for production invocations. On activation failure the engine restores the backup manifest in the same process that owns the deployment lock, so automatic rollback cannot self-deadlock on Linux. It removes only newly created managed files, records any parent directories created by the release, and removes those directories only when still empty. `--rollback BACKUP_DIR` is a separate, lock-protected manual operation. A `flock` lock outside DocumentRoot rejects concurrent activation or manual rollback on Linux. Failure injection (`--failure-point assets`, `prerender`, `root`, or `htaccess`) exists for temporary-fixture tests only. This phase intentionally does not upload, connect to REG.RU, create secrets, or perform production activation.
